...
class DropTracker {
  ...
  saveDrop(accountId, item) {
    db.run(`
      INSERT INTO drops (account_id, item_name, item_type, assetid, classid, market_price)
      VALUES (?, ?, ?, ?, ?, ?)
  ...
  getAll(limit = 200) {
    return db.all(`
      SELECT d.id, d.item_name, d.item_type, d.market_price, d.dropped_at,
  ...
  getStats() {
    ...
    const byDay   = db.all(`
      SELECT DATE(dropped_at) AS date, COUNT(*) AS count, SUM(market_price) AS revenue
      FROM drops WHERE dropped_at >= datetime('now', '-30 days')
      GROUP BY DATE(dropped_at) ORDER BY date ASC
    `)
    const topAccounts = db.all(`
      SELECT a.login, COUNT(d.id) AS drops, SUM(d.market_price) AS revenue
      FROM drops d JOIN accounts a ON d.account_id = a.id
      GROUP BY a.id ORDER BY drops DESC LIMIT 10
    `)
...
module.exports = new DropTracker()
